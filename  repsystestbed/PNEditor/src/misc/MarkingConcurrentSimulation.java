...
public class MarkingConcurrentSimulation
{
	...
	public static void main(String[] args) throws InterruptedException
	{
		...
		try
		{
			Document document = new DocumentImporter().readFromFile(new File(
					"src/misc/MarkingConcurrentSimulation.pflow"));
		...
		{
			Logger.getLogger(MarkingConcurrentSimulation.class.getName()).log(
	...
	private static void doTest(final PetriNet petriNet)
			...
			public void run()
			{
				...
				for (int i = 0; i < 100; i++)
				{
					try
					{
						...
						for (Place place : petriNet.getRootSubnet()
								.getPlacesRecursively())
						{
					...
					{
						Logger.getLogger(
								MarkingConcurrentSimulation.class.getName())
