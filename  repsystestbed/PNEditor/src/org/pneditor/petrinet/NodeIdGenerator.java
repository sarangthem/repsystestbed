...
public class NodeIdGenerator
{
	...
	public void setUniqueId(Node node)
	{
		...
		} else
		{
			throw new RuntimeException(
					"Node which is not Place, Transition, Subnet neither ReferencePlace");
		}
		node.setId(id);
	}
	...
	public void fixFutureUniqueIds(Subnet rootSubnet)
	{
		...
		for (ReferencePlace referencePlace : rootSubnet
				.getReferencePlacesRecursively())
		{
			...
			if (referencePlaceLabel.startsWith("rp"))
			{
				try
				{
					int referencePlaceNumber = Integer
							.parseInt(referencePlaceLabel.substring(1));
