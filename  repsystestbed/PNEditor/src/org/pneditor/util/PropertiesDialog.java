...
public class PropertiesDialog<E> extends JDialog implements ActionListener
{
	...
	public PropertiesDialog(Frame parent, E e)
	{
		...
		{
			Logger.getLogger(PropertiesDialog.class.getName()).log(
		...
		if (e.getSource() == saveButton)
		{
			try
			{
				for (Field field : controls.keySet())
				{
					if (field.getType() == boolean.class)
					{
						field.setBoolean(this.e,
								((JCheckBox) controls.get(field)).isSelected());
					} else if (field.getType() == String.class)
					{
						field.set(this.e,
								((JTextField) controls.get(field)).getText());
					}
				}
			} catch (IllegalArgumentException ex)
			{
