...
public class ViptoolPnmlFileType extends FileType
{
	...
	public Icon getIcon()
	{
		final Icon icon = GraphicsTools
				.getIcon("pneditor/filechooser/pnml.gif");
		return icon;
	}
	...
	public void save(Document document, File file) throws FileTypeException
	{
		try
		{
			final InputStream xslt = getClass().getResourceAsStream(
					"/xslt/pnml-export.xslt");
			PetriNet petriNet = document.petriNet;
			Marking initialMarking = petriNet.getInitialMarking();
			new DocumentExporter(document, initialMarking).writeToFileWithXslt(
					file, xslt);
	...
	public Document load(File file) throws FileTypeException
	{
		...
		{
			final InputStream xslt = getClass().getResourceAsStream(
					"/xslt/pnml-import.xslt");
			Document document = new DocumentImporter().readFromFileWithXslt(
					file, xslt);
			document.petriNet.getRootSubnet()
					.setViewTranslationToCenterRecursively();
			return document;
		} catch (JAXBException ex)
		{
			if (!file.exists())
