...
public class ReplaceSubnetAction extends AbstractAction
{
	...
	public ReplaceSubnetAction(Root root)
	{
		...
		putValue(SMALL_ICON,
				GraphicsTools.getIcon("pneditor/replacesubnet.gif"));
		setEnabled(false);
	}
	...
	public void actionPerformed(ActionEvent e)
	{
		if (root.getClickedElement() instanceof Subnet
				|| !root.getSelection().getSubnets().isEmpty())
		{
			...
			if (chooser.showDialog(root.getParentFrame(), "Choose") == JFileChooser.APPROVE_OPTION)
			{
				...
				try
				{
					...
					for (Element element : root
							.getSelectedElementsWithClickedElement())
					{
					...
					Set<Subnet> previousSubnets = root.getDocument().petriNet
					...
					if (looseMethodCheckBox.isSelected())
					{
						root.getUndoManager()
								.executeCommand(
										new ReplaceSubnetsLooseMethodCommand(
												selectedSubnets, storedSubnet,
												petriNet));
					} else
					{
						root.getUndoManager().executeCommand(
								new ReplaceSubnetsCommand(selectedSubnets,
										storedSubnet, petriNet));
					}

					Set<Subnet> createdElements = root.getDocument().petriNet
							.getCurrentSubnet().getSubnets();
					...
					// root.getUndoManager().executeCommand(new
					// ReplaceSubnetCommand(clickedSubnet, storedSubnet));
