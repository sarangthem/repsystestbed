...
class EdgeZigzagFeature implements Feature
{
	...
	public void mousePressed(MouseEvent event)
	{
		...
		if (mouseButton == MouseEvent.BUTTON1
				...
				&& (root.isSelectedTool_Select() || root.isSelectedTool_Place()
						|| root.isSelectedTool_Transition()
						|| root.isSelectedTool_Arc() || root
						.isSelectedTool_Token()
						&& !(root.getClickedElement() instanceof PlaceNode))
				&& root.getClickedElement() instanceof ArcEdge)
		{
			...
			edge = (Edge) root.getDocument().petriNet.getCurrentSubnet()
					.getElementByXY(x, y);

			oldBreakPoints = edge.getBreakPointsCopy();
			startingMouseLocation = new Point(x, y);
			activeBreakPoint = edge.addOrGetBreakPoint(new Point(
					startingMouseLocation));
			started = true;
		}
	}
		...
		if (started)
		{
			...
			if (change)
			{
				...
				root.getUndoManager().executeCommand(
						new SetEdgeZigzagPointCommand(edge,
								startingMouseLocation, targetLocation));
			}
			started = false;
		}
	}
	...
	{
		if (root.isSelectedTool_Select() || root.isSelectedTool_Place()
				|| root.isSelectedTool_Transition()
				|| root.isSelectedTool_Arc() || root.isSelectedTool_Token())
		{
			Element element = root.getDocument().petriNet.getCurrentSubnet()
					.getElementByXY(x, y);
