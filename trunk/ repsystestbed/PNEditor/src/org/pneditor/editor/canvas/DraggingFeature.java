...
class DraggingFeature implements Feature
{
	...
	private int prevDragX; // During dragging, these record the x and y
							// coordinates of the
		...
		if (!doubleclick)
		{
			if (mouseButton == MouseEvent.BUTTON1
					...
					&& (root.isSelectedTool_Select()
							|| root.isSelectedTool_Place() || root
								.isSelectedTool_Transition())
					&& root.getClickedElement() instanceof Node)
			{
				...
				draggedElement = root.getDocument().petriNet.getCurrentSubnet()
						.getElementByXY(x, y);
		...
		if (!deltaPosition.equals(new Point(0, 0)))
		{
			if (!root.getSelection().isEmpty())
			{
				...
				root.getUndoManager().executeCommand(
						new MoveElementsCommand(root.getSelection()
								.getElements(), deltaPosition));
			} else
			{
				draggedElement.moveBy(-deltaPosition.x, -deltaPosition.y); // move
																			...
																			// position
				root.getUndoManager().executeCommand(
						new MoveElementCommand(draggedElement, deltaPosition));
			}
		}
	}
	...
	public void setCursor(int x, int y)
	{
				...
				|| root.isSelectedTool_Transition())
		{
			...
			{
				canvas.alternativeCursor = Cursor
