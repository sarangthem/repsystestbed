		...
		MouseMotionListener, MouseWheelListener
{
	...
	{
		return root.getDocument().petriNet.getCurrentSubnet()
				.getViewTranslation().x + getWidth() / 2;
	}

	public int getTranslationY()
	{
		return root.getDocument().petriNet.getCurrentSubnet()
				.getViewTranslation().y + getHeight() / 2;
	}
	...
	public void mousePressed(MouseEvent event)
	{
		...
		event = new MouseEvent((Component) event.getSource(), event.getID(),
				event.getWhen(), event.getModifiers(), x, y,
				event.getXOnScreen(), event.getYOnScreen(),
				event.getClickCount(), event.isPopupTrigger(),
				event.getButton());

		root.setClickedElement(root.getDocument().petriNet.getCurrentSubnet()
				.getElementByXY(x, y));
