...
public class ReplaceSubnetLooseMethodCommand implements Command
{
	...
	public ReplaceSubnetLooseMethodCommand(Subnet subnet, Subnet storedSubnet,
			PetriNet petriNet)
	{
		this.subnet = subnet;
	...
	private void setArcMultiplicitiesToOne(Subnet subnet)
	{
		...
		{
			Arc outArc = CollectionTools.getFirstElement(referencePlace
					.getConnectedArcs(true));
			if (outArc != null)
			{
			...
			Arc inArc = CollectionTools.getFirstElement(referencePlace
					.getConnectedArcs(false));
			if (inArc != null)
			{
