...
public abstract class PlaceNode extends Node
{
	...
	public Arc getConnectedArc(Transition transition, boolean placeToTransition)
	{
		...
		{ // TODO: !!! what if the arc is hidden behind ReferenceArc inside
			// Subnet?
	...
	protected void drawStaticShadow(Graphics g)
	{
		g.setColor(color);
		final int phase = 4;
		g.fillOval(getStart().x + phase, getStart().y + phase, getWidth() - 1,
				getHeight() - 1);
	}
	...
	protected void drawTokens(Graphics g, Marking marking)
	{
		...
		} else if (marking.getTokens(this) > 9)
		{
			GraphicsTools.drawString(g,
					Integer.toString(marking.getTokens(this)), x, y,
