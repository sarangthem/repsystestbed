...
public class ElementCloner
{
	...
	public static <E extends Element> Set<E> getClones(Collection<E> elements,
			PetriNet petriNet)
	{
	...
	private Place clonePlace(Place place)
	{
		...
		petriNet.getInitialMarking().setTokens(newPlace,
				petriNet.getInitialMarking().getTokens(place));
		return newPlace;
	}
	...
	{
		Arc newArc = new Arc(getClone(arc.getPlaceNode()),
				getClone(arc.getTransition()), arc.isPlaceToTransition());
