...
public class ETNoPretrusted extends Algorithm
{
	...
	static Logger logger = Logger.getLogger(ETNoPretrusted.class.getName());
	
	
	
	public ETNoPretrusted()
	{
	...
	public void setConfig(Properties config)
	{
		if(config != null)
		{
			try
			{
				...
				Integer iterations = new Integer(config.getProperty(ETNoPretrusted.NUMITERATIONS));
...
//						else sij--;
			...
			double[][] e = new double[numVertices][numVertices];
			for(int i=0;i<numVertices;i++)
			{
				for(int j=0;j<numVertices;j++) e[i][j] = tempScore;
			}
//			Matrix tkMatrix = new Matrix(e);
			
			Matrix tkplus1Matrix = null;
			Matrix tkMatrix = trustScores;
